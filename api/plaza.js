...
function getRoom(id) {
  ...
  if (!room) {
    ...
    // stations = activity state keyed by station id (chess/go/sandbox/...),
    //   stored opaquely: the server relays + persists, clients own the rules.
    room = { agents: new Map(), watchers: new Set(), pendingLeft: new Map(), stations: new Map(), msgs: [], seq: 0 };
    rooms.set(id, room);
  }
  return room;
}

function stationSnapshot(room) {
  return [...room.stations.entries()].map(([id, s]) => ({ id, kind: s.kind, seats: s.seats, state: s.state, version: s.version }));
}

function getStation(room, id, kind) {
  let s = room.stations.get(id);
  if (!s) {
    s = { kind: kind || 'unknown', seats: {}, state: {}, version: 0 };
    room.stations.set(id, s);
  }
  return s;
}

/** Drop an agent from every station seat (on true departure). */
function freeSeats(room, agentId) {
  for (const [id, s] of room.stations) {
    let changed = false;
    for (const slot of Object.keys(s.seats)) {
      if (s.seats[slot] === agentId) {
        delete s.seats[slot];
        changed = true;
      }
    }
    if (changed) {
      s.version += 1;
      broadcast(room, { t: 'st', id, kind: s.kind, seats: s.seats, state: s.state, version: s.version });
    }
  }
}
...
const server = http.createServer((req, res) => {
  ...
  if (STATIC_DIR && url.pathname !== '/api/plaza') {
    ...
    fs.readFile(file, (err, body) => {
      ...
      // the HTML shell must always revalidate (so a new build is picked up);
      // hashed assets are safe to cache hard.
      const ext = path.extname(file);
      const cache = ext === '.html' ? 'no-cache' : ext === '.js' || ext === '.css' ? 'public, max-age=31536000, immutable' : 'no-cache';
...
wss.on('connection', (ws, req) => {
  ...
  send(ws, { t: 'roster', room: roomId, agents: [...room.agents.values()].map(publicAgent), stations: stationSnapshot(room) });
  ...
  ws.on('message', (raw) => {
    ...
    // ---- activity stations: seats + opaque shared state ----
    if (data.t === 'st-join' || data.t === 'st-leave' || data.t === 'st-set' || data.t === 'st-patch') {
      const id = String(data.id ?? '').slice(0, 40);
      if (!/^[a-z0-9_:-]{1,40}$/i.test(id)) return;
      const s = getStation(room, id, String(data.kind ?? '').slice(0, 20));
      if (data.t === 'st-join') {
        const slot = String(data.slot ?? '').slice(0, 20);
        // free any seat this agent already holds here, then take the requested one
        for (const k of Object.keys(s.seats)) if (s.seats[k] === agent.id) delete s.seats[k];
        if (slot) {
          if (s.seats[slot] && s.seats[slot] !== agent.id) {
            send(ws, { t: 'st-error', id, code: 'seat-taken', slot });
            return;
          }
          s.seats[slot] = agent.id;
        }
      } else if (data.t === 'st-leave') {
        for (const k of Object.keys(s.seats)) if (s.seats[k] === agent.id) delete s.seats[k];
      } else if (data.t === 'st-set') {
        if (data.state && typeof data.state === 'object') s.state = data.state;
      } else if (data.t === 'st-patch') {
        if (data.patch && typeof data.patch === 'object') s.state = { ...s.state, ...data.patch };
      }
      s.version += 1;
      broadcast(room, { t: 'st', id, kind: s.kind, seats: s.seats, state: s.state, version: s.version });
      return;
