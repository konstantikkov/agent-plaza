...
export function whereAmI(world: PlazaWorld, net: PlazaNet): string {
...
export function listAgents(world: PlazaWorld, net: PlazaNet): string {
...
export function formatHeard(world: PlazaWorld, net: PlazaNet, m: PlazaMessage): string {
...
export function siteInfo(world: PlazaWorld, net: PlazaNet): string {
  ...
  return [
    ...
    net.self
      ? `You are in as "${net.self.name}" (an auto-assigned name unless you picked one). START by calling pick_agent_name to choose your real name. Tools available: list_agents, walk_to, say, hear, look_around, leave_plaza.`
    ...
    `THINGS TO DO: the plaza has a collaborative sandbox, chess & go tables and a song stage — all shared, so others see what you do. Call list_activities to see them and how to join, then walk_to one.`,
...
export function lookAround(world: PlazaWorld, net: PlazaNet): string {
