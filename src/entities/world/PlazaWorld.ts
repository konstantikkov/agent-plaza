...
export class PlazaWorld {
  ...
  constructor(
    ...
    private opts: { overrides?: LayoutOverrides; reserveAreas?: { x0: number; z0: number; x1: number; z1: number }[] } = {},
  ) {
  ...
  private init(): void {
    ...
    this.buildLevels();
    this.buildGround();
    // reserve activity-station footprints early so trees/buildings skip them
    for (const a of this.opts.reserveAreas ?? []) {
      for (let z = Math.max(4, a.z0); z <= Math.min(N - 5, a.z1); z++) {
        for (let x = Math.max(4, a.x0); x <= Math.min(N - 5, a.x1); x++) {
          this.reserveCells(`rsv-${x}-${z}`, x, z, 1, 1);
        }
      }
    }
  ...
  /** The surface group activity builders add their structures to. */
  getSurface(): THREE.Group {
    return this.worldSurface;
  }

  /** Flatten rectangles of terrain to a level, then rebuild the ground once. */
  flattenAreas(areas: { x0: number; z0: number; x1: number; z1: number; level: number }[]): void {
    for (const a of areas) {
      const lvl = Math.max(0, Math.min(8, Math.round(a.level)));
      for (let z = Math.max(0, a.z0); z <= Math.min(N - 1, a.z1); z++) {
        for (let x = Math.max(0, a.x0); x <= Math.min(N - 1, a.x1); x++) {
          this.levels[z * N + x] = lvl as never;
        }
      }
    }
    this.buildGround();
  ...
  /** Block cells so avatars can't walk through a solid structure. */
  reserveCells(id: string, x0: number, z0: number, w: number, d: number): void {
    try {
      this.grid.place(id, x0, z0, { width: w, depth: d });
    } catch {
      /* already claimed */
    }
  }

  /** Release an activity footprint reserved at build time, and any water there,
   *  so the flattened floor is walkable. */
  openArea(area: { x0: number; z0: number; x1: number; z1: number }): void {
    for (let z = area.z0; z <= area.z1; z++) {
      for (let x = area.x0; x <= area.x1; x++) {
        this.grid.remove(`rsv-${x}-${z}`);
        this.grid.remove(`water-${x}-${z}`);
      }
    }
  }

  /** Add a named place so walk_to / look_around can find an activity. */
  registerPlace(kind: string, x: number, z: number): void {
    this.extraPlaces.push({ kind, x, z });
  }
  ...
  getPlaces(): { kind: string; x: number; z: number }[] {
    ...
    for (const p of this.extraPlaces) places.push({ ...p });
    return places;
  }

  private extraPlaces: { kind: string; x: number; z: number }[] = [];
